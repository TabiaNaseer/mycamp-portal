        ...
        <div class="page-banner ovbl-dark" style="background-image:url(assets/images/event/6pic.jfif);">
            <div class="container">
                <div class="page-banner-entry">
                    <h1 class="text-white">Community Service</h1>
				 </div>
            </div>
        </div>
		<!-- Breadcrumb row -->
...
$query1 = "SELECT * FROM community ORDER BY id DESC LIMIT {$offset},{$limit}";
$run1 = mysqli_query($db,$query1);;
while($community=mysqli_fetch_array($run1)){
 ?>
							<li class="action-card col-lg-4 col-md-6 col-sm-12 col-xs-12 m-b40"  >
								<div class="event-bx m-b30"  style="height: 500px; ">
									<div class="action-box" >
										<img src="assets/images/event/<?=$community['c_pic']?>" style="width: 360px; height: 200px" alt="">
									</div>
									<div class="info-bx d-flex" style="width: 360px; height: 330px">
										
										
										<div class="event-info">
											<h4 class="event-title"><?=$community['c_title']?></h4>

											
											<p ><i class="	fa fa-file-text-o"></i>	&nbsp;	&nbsp;<?=$community['c_desc']?></p>
										</div>
									</div>
					...
					if (mysqli_num_rows($result1)>0){
						...
						if($page > 1){
						echo '<li class="previous"><a href="communityservice.php?page='.($page-1).'" ><i class="ti-arrow-left"></i>Prev</a></li>';
					}
						for($i =1; $i<=$total_page; $i++){
							...
							echo '<li class = "'.$active.'"><a href="communityservice.php?page='.$i.'">'.$i.'</a></li>';


						}
						if($total_page > $page){
						echo '<li  class="next"><a href="communityservice.php?page='.($page+1).'" >Next <i class="ti-arrow-right"></i></a></li>';
				}
						echo'</ul>';
					}
